...
var keyBytes=string.IsNullOrWhiteSpace(signingKey)?RandomNumberGenerator.GetBytes(32):Encoding.UTF8.GetBytes(signingKey);
builder.Services.AddAuthentication(JwtBearerDefaults.AuthenticationScheme).AddJwtBearer(options=>
{
    options.MapInboundClaims=false;
    options.TokenValidationParameters=new TokenValidationParameters{ValidateIssuerSigningKey=true,IssuerSigningKey=new SymmetricSecurityKey(keyBytes),ValidateIssuer=true,ValidIssuer=builder.Configuration["Jwt:Issuer"]??"Hambaft",ValidateAudience=true,ValidAudience=builder.Configuration["Jwt:Audience"]??"Hambaft.Clients",ValidateLifetime=true,ClockSkew=TimeSpan.FromSeconds(30),RoleClaimType="client_role"};
...
app.MapGet("/api/sessions/{sessionId:guid}",async(Guid sessionId,SessionRuntime runtime,CancellationToken ct)=>await runtime.GetSessionAsync(sessionId,ct) is { } view?Results.Ok(view):Results.NotFound());
