...
public static class ViewProjector
{
    ...
    private static TeamScenePresentationView? TeamScene(StoryPackage package,string? checkpointId,IReadOnlyList<StoryletAssignment> assignments,IReadOnlyList<SubmittedStoryChoice> submissions,Team team,WorldEntity? entity,IReadOnlyList<AvailableInteractionView> availableInteractions,IReadOnlyList<WorldEntity> entities,IReadOnlyList<StoryMemory> memories,string? locale)
    {
        ...
        var authored=package.PresentationDefinition.TeamSceneDefinitions.SingleOrDefault(x=>x.CheckpointId==checkpointId&&x.EntityDefinitionId==entity.DefinitionId);
        ...
        var business=authored.BusinessActivity is null?null:new TeamBusinessActivityPresentationView(authored.EntityDefinitionId,authored.BusinessActivity.Title,authored.BusinessActivity.Summary,authored.BusinessActivity.Implication,authored.BusinessActivity.UnlockedActions);
        ...
        return new(authored.SceneId,authored.Title,authored.OpeningNarrative,authored.Objective,authored.RequiredInvestigationCount,authored.InvestigationLocations.Select(x=>new InvestigationLocationPresentationView(x.Id,x.Title,x.Identity,x.Narrative,x.Evidence.Select(e=>new InvestigationEvidencePresentationView(e.Id,e.Title,e.SourceLabel,e.Description,e.WhyItMatters,e.Certainty,e.Unlocks)).ToList())).ToList(),business,pacts,decision,reactions,progress.Select(x=>x.LocationId).Distinct(StringComparer.Ordinal).ToList(),progress.SelectMany(x=>x.EvidenceIds).Distinct(StringComparer.Ordinal).ToList());
