...
public sealed class EffectEngine : IEffectEngine
{
    ...
    private static Team CurrentTeam(EffectExecutionContext x)=>x.Assignment?.TargetTeamId is { } id?x.State.Teams.Single(t=>t.Id==id):throw new DomainException("Effect requires a current Team.");
    private static WorldEntity CurrentEntity(EffectExecutionContext x)
    {
        var id=x.Assignment?.TargetEntityId??(x.Assignment?.TargetTeamId is { } team?x.State.Teams.Single(t=>t.Id==team).ControlledEntityId:null);
