...
public sealed partial class SessionRuntime
{
    ...
    public async Task<AdminSessionView?> GetAdminAsync(Guid sessionId,CancellationToken ct)
    {
        ...
        var preview=package.PresentationDefinition.MetricBands.Count==0?null:SemanticPresentation.Project(package,state.Id,state.CurrentCheckpointId,state.Entities,state.Metrics.Where(x=>x.Scope==MetricScope.World).ToList(),[],false).World;
        return new(state.Id,state.StoryletAssignments.Where(x=>x.RequiredResponse&&x.Status==StoryletAssignmentStatus.Assigned&&x.TargetTeamId is not null).Select(x=>x.TargetTeamId!.Value).Distinct().OrderBy(x=>x).ToList(),state.StoryletAssignments.Where(x=>x.Status is StoryletAssignmentStatus.Assigned or StoryletAssignmentStatus.Responded).Select(x=>x.StoryletId).ToList(),state.CurrentCheckpointId,state.Proposals.Where(x=>x.Status is ProposalStatus.Pending or ProposalStatus.Countered).ToList(),state.Agreements.Where(x=>x.Status==AgreementStatus.Active).ToList(),state.ScheduledConsequences.Where(x=>x.Status==ScheduledConsequenceStatus.Pending).ToList(),state.AuthoredBehaviorSelections.ToList(),diagnostics,state.StoryVersion,state.ContentHash,state.StateVersion,state.Metrics.ToList(),preview);
    }
