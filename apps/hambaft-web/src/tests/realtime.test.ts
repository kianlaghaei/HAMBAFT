...
it('SignalR events invalidate only relevant authoritative REST projections', () => {
  ...
  expect(invalidationKeysForEvent('AuthoredBehaviorResolved', ids.session)).toContainEqual(['public-world', ids.session])
  expect(invalidationKeysForEvent('AgreementExecuted', ids.session)).toContainEqual(['public-world', ids.session])
