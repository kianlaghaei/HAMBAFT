...
export const api = {
  ...
  recordInvestigation: (locationId: string, evidenceIds: string[], expectedStateVersion: number, token: string): Promise<CommandResponse> => request('/api/story/investigations', { method: 'POST', token, body: { locationId, evidenceIds, expectedStateVersion, commandId: crypto.randomUUID() }, schema: commandSchema }),
