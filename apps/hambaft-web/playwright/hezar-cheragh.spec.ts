...
test('real two-Team Hezar Cheragh playable slice preserves privacy through World Ending', async ({ browser, page: admin }) => {
  ...
  await expect(teamA.locator('.location-list button')).toHaveCount(13)
  await expect(teamA.getByText('صبح', { exact: false }).first()).toBeVisible()
  await expect(teamA.getByText(/بازار زیر فشار است|نگرانی در بازار پیچیده|بازار هنوز آرام است/).first()).toBeVisible()
  ...
  await mapTarget.focus(); await teamA.keyboard.press('Enter')
  await expect(mapTarget).toHaveAttribute('aria-checked', 'true')
  ...
  await expect(teamA.getByText(/شب|غروب|بعدازظهر|نزدیک ظهر|صبح/).first()).toBeVisible()
